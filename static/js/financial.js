/* ABC — Module 5: Financial Scenario Planning */
...
async function loadFinancialDashboard() {
  ...
  try {
    const [kpis, forecast] = await Promise.all([
      ABC.apiFetch('/api/financial/kpis' + qs),
      ABC.apiFetch('/api/financial/forecast-profitability' + qs),
    ]);
...
function renderFinancialKPIs(kpis) {
  ...
  set('fin-kpi-revenue',    ABC.fmt.gbpM(kpis.total_revenue_gbp));
  set('fin-kpi-cost',       ABC.fmt.gbpM(kpis.total_cost_gbp));
  set('fin-kpi-margin',     ABC.fmt.gbpM(kpis.total_margin_gbp));
  set('fin-kpi-margin-pct', ABC.fmt.pct(kpis.margin_pct));
  set('fin-kpi-cpp',        ABC.fmt.gbp(kpis.avg_cost_per_completion));
...
function renderMonthlyChart(trend) {
  ...
  ABC.destroyChart('fin-monthly');
  ABC.registerChart('fin-monthly', new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
      datasets: [
        ...
        { label: 'Margin %',   data: trend.map(t => t.margin_pct),borderColor: ABC.colors.accent, type: 'line', fill: false, tension: 0.4, pointRadius: 0, yAxisID: 'y1' },
      ],
    },
    options: {
      ...
      scales: {
        ...
        y:  { ...ABC.chartDefaults.scales.y, ticks: { ...ABC.chartDefaults.scales.y.ticks, callback: v => '£' + (v/1000).toFixed(0) + 'k' } },
...
function renderJobTypeChart(breakdown) {
  ...
  ABC.destroyChart('fin-jobtype');
  ABC.registerChart('fin-jobtype', new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
      datasets: [
...
function renderForecastProfit(data) {
  ...
  ABC.destroyChart('forecast-profit');
  ABC.registerChart('forecast-profit', new Chart(ctx, {
    type: 'line',
    data: {
      labels: mf.map(m => m.month),
      datasets: [
        { label: 'Forecast Revenue GBP', data: mf.map(m => m.revenue), borderColor: ABC.colors.ok,      fill: false, tension: 0.4, pointRadius: 0, borderDash: [5,3] },
        { label: 'Forecast Cost GBP',    data: mf.map(m => m.cost),    borderColor: ABC.colors.crit,    fill: false, tension: 0.4, pointRadius: 0, borderDash: [5,3] },
        { label: 'Forecast Margin %',   data: mf.map(m => m.margin_pct), borderColor: ABC.colors.accent, fill: false, tension: 0.4, pointRadius: 3, yAxisID: 'y1' },
      ],
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      ...
      scales: {
        ...
        y1: { ...ABC.chartDefaults.scales.y, position: 'right', grid: { display: false },
              min: y1Min, max: y1Max,
              ticks: { ...ABC.chartDefaults.scales.y.ticks, callback: v => v.toFixed(1) + '%' } },
      },
    },
  }));
}
...
async function runScenario() {
  const payload = {
    ...
    region_code: ABC.getRegion() || null,
  };

  const panel = document.getElementById('scenario-results');
  if (panel) panel.style.display = 'block';
  ABC.setLoading('waterfall-chart', true);
  ...
  try {
    ...
    ABC.setLoading('waterfall-chart', false);
  }
}
...
function renderScenarioResults(data) {
  ...
  if (title) {
    ...
    ABC.hydrateIcons(title.parentElement || document);
  }
  set('sc-res-revenue',    ABC.fmt.gbpM(data.revenue_gbp));
  set('sc-res-cost',       ABC.fmt.gbpM(data.total_cost_gbp));
  set('sc-res-margin',     ABC.fmt.gbpM(data.margin_gbp));
  set('sc-res-margin-pct', ABC.fmt.pct(data.margin_pct));
  set('sc-res-cpp',        ABC.fmt.gbp(data.cost_per_completion));
  ...
  if (ctx && data.waterfall) {
    ...
    ABC.destroyChart('waterfall');
    ABC.registerChart('waterfall', new Chart(ctx, {
      type: 'bar',
      data: {
      ...
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { ...ABC.chartDefaults.plugins, legend: { display: false } },
        scales: {
          ...ABC.chartDefaults.scales,
          y: { ...ABC.chartDefaults.scales.y, ticks: { ...ABC.chartDefaults.scales.y.ticks, callback: v => '£' + (v/1000).toFixed(0) + 'k' } },
        },
