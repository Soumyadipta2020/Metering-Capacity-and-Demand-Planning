/* ABC — Module 2: Contact Centre Forecasting */
...
async function loadForecastingDashboard(force = false) {
  const region = ABC.getRegion();
  const planningQs = ABC.getGlobalQs({ year: 2025 });
  ...
  try {
    const [planningKpis, kpis, funnel] = await Promise.all([
      ABC.apiFetch('/api/forecasting/planning-target-kpis' + planningQs),
      ABC.apiFetch('/api/forecasting/channel-kpis' + planningQs),
      ABC.apiFetch('/api/forecasting/funnel' + planningQs),
    ]);
...
function configureForecastPlanningCards() {
  ...
  ABC.hydrateIcons?.(document.getElementById('forecast-kpis') || document);
}
...
function renderForecastPlanningKPIs(kpis) {
  ...
  const signedNumber = (value) => `${value >= 0 ? '+' : ''}${ABC.fmt.num(value)}`;
  const signedRateGap = (value) => `${value >= 0 ? '+' : ''}${value.toFixed(1)}`;

  setForecastKPI('fc-kpi-accuracy', ABC.fmt.pct(accuracy));
  ...
  setForecastDelta('fc-kpi-visits-delta', `${ABC.fmt.num(visits)} actual vs ${ABC.fmt.num(visitTarget)} target`, visitTone);
  ...
  setForecastDelta('fc-kpi-success-delta', `${ABC.fmt.pct(success)} actual vs ${ABC.fmt.pct(successTarget)} target`, successTone);
  ...
  setForecastDelta('fc-kpi-fallout-delta', `${ABC.fmt.pct(fallout)} actual vs ${ABC.fmt.pct(falloutTarget)} target`, falloutTone);
...
function renderForecastKPIs(data, forecastValues) {
  ...
  setForecastKPI('fc-kpi-volume', ABC.fmt.num(forecastContacts));
  setForecastKPI('fc-kpi-bookings', ABC.fmt.num(projectedVisits));
  setForecastKPI('fc-kpi-conversion', ABC.fmt.pct(visitRate));
  setForecastKPI('fc-kpi-abandon', ABC.fmt.pct(_forecastPlanningRates.abandonRate));
}
...
function renderChannelBreakdown(kpis) {
  ...
  if (ctx && channels.length) {
    ...
    ABC.destroyChart('channel-breakdown');
    ABC.registerChart('channel-breakdown', new Chart(ctx, {
      type: 'doughnut',
      data: {
  ...
  if (tbody) {
    tbody.innerHTML = channels.map(c => `
      <tr>
        ...
        <td>${ABC.fmt.num(c.volume)}</td>
        <td>${ABC.fmt.num(c.bookings)}</td>
        <td><strong class="text-ok">${ABC.fmt.pct(c.conversion_pct)}</strong></td>
        <td><span class="text-warn">${ABC.fmt.pct(c.abandon_pct)}</span></td>
      </tr>
    `).join('');
  }
}
...
async function loadForecast() {
  ...
  const qs = ABC.getGlobalQs({ channel, weeks: 52 });
  ABC.setLoading(['forecast-chart', 'model-accuracy-body', 'model-comparison-chart'], true);

  try {
    const data = await ABC.apiFetch('/api/forecasting/forecast' + qs);
    if (!data) return;
    ...
    ABC.setLoading(['forecast-chart', 'model-accuracy-body', 'model-comparison-chart'], false);
  }
}
...
function onForecastModelChange() {
  ABC.setLoading('forecast-chart', true);
  if (_lastForecastData) {
    renderForecastChart(_lastForecastData);
    requestAnimationFrame(() => ABC.setLoading('forecast-chart', false));
  } else {
    loadForecast();
  }
}
...
function updateForecastTitle(activeModel) {
  ...
  ABC.hydrateIcons?.(title.parentElement || title);
}
...
function renderForecastChart(data) {
  ...
  const centralColor = activeModel ? (modelColors[activeModel] || ABC.colors.accent) : ABC.colors.accent;
  const isLightTheme = ABC.getTheme?.() !== 'dark';
  ...
  ABC.destroyChart?.('forecast');
  ABC.registerChart('forecast', new Chart(ctx, {
    type: 'line',
    data: {
      labels: weekLabels,
      datasets: [
    ...
    options: {
      ...
      scales: {
        ...
        x: { ...ABC.chartDefaults.scales.x, ticks: { ...ABC.chartDefaults.scales.x.ticks, maxTicksLimit: 13 } },
      },
    },
  }));
}
...
function renderModelComparison(data) {
  ...
  const datasets = Object.entries(data.model_forecasts).map(([m, vals]) => ({
    ...
    borderColor: modelColors[m] || ABC.colors.accent,
    fill: false, tension: 0.4, pointRadius: 0, borderWidth: 1.5,
  }));
  ABC.destroyChart('model-comparison');
  ABC.registerChart('model-comparison', new Chart(ctx, {
    type: 'line',
...
async function loadConversionTrend() {
  ABC.setLoading('conversion-trend-chart', true);
  const funnel = await ABC.apiFetch('/api/forecasting/funnel' + ABC.getGlobalQs());
  if (!funnel) {
    ABC.setLoading('conversion-trend-chart', false);
    return;
  }
  ...
  ABC.destroyChart('conversion-trend');
  ABC.registerChart('conversion-trend', new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
      datasets: [
        ...
        { label: 'Success Rate %',         data: cr,     borderColor: ABC.colors.accent, type: 'line', fill: false, tension: 0.4, pointRadius: 0, yAxisID: 'y1' },
      ],
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      ...
      scales: {
        ...ABC.chartDefaults.scales,
        y:  { ...ABC.chartDefaults.scales.y, position: 'left' },
...
async function loadForecastingOverview(showLoading = true) {
  if (showLoading) ABC.setLoading('channel-breakdown-chart', true);
  try {
    const kpis = await ABC.apiFetch('/api/forecasting/channel-kpis' + ABC.getGlobalQs());
    if (kpis) renderChannelBreakdown(kpis);
    if (kpis) _forecastTabLoadKeys.set('overview', getForecastTabLoadKey('overview'));
  } finally {
    if (showLoading) ABC.setLoading('channel-breakdown-chart', false);
  }
}
